...
from lightprop2d import Beam2D, gaussian_beam, mm, cm, m, um, round_hole
import pyMMF
import numpy as np
...
slm = slm_phaseprofile(area_size / npoints, 2 * np.pi * np.random.random(size=(100, 100)),
...
fbeam = Beam2D(ibeam.area_size, npoints,
               wl, init_field=ibeam._np(ibeam.field), use_gpu=0, unsafe_fft=True)
